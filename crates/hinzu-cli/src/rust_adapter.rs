...
pub fn extract_facts(project: &Path) -> Result<FactSet> {
    ...
    run_extraction(project, &driver, &sysroot, &facts_dir)?;
...
fn run_extraction(project: &Path, driver: &Path, sysroot: &Path, facts_dir: &Path) -> Result<()> {
...
/// Whether `path` is one of the driver's `facts-*.json` output files.
fn is_facts_file(path: &Path) -> bool {
    path.extension().is_some_and(|e| e == "json")
        && path
            .file_name()
            .and_then(|n| n.to_str())
            .is_some_and(|n| n.starts_with("facts-"))
}
